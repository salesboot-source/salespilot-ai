...
import { generateEmail, IntelligenceReport, ResearchOutput } from '@/lib/ai';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const emailOutput = await generateEmail({
    ...
    const result = await sql`
      INSERT INTO outreach_emails (user_id, research_id, company_name, subject, body, tone)
      VALUES (${userId}, ${research_id}, ${research[0].company_name}, ${emailOutput.subject}, ${emailOutput.body}, ${tone || 'formal'})
      RETURNING *
    `;

    return Response.json({ success: true, message: 'Email ready!', data: result[0] }, { status: 201 });
  } catch (error) {
    console.error('Email error:', error);
    return Response.json({ success: false, message: "We couldn't generate the email. Please try again.", errors: null }, { status: 500 });
  }
}
